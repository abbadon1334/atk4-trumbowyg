...
class Context extends \Atk4\Ui\Behat\Context
{
     ...
     */
    public function iTypeInEditor(string $selector, string $text): void
    {
        ...
        $session->executeScript("$('" . $selector . "').innerHTML = '" . $text . "';");
        $session->executeScript("$('" . $selector . "').trigger('keyup')");
    }
    ...
    /**
     * @Then I check if editor :selector value is equal to :text
     */
